...
class HCADParcelLookup:
    ...
    def _load_real_acct(self, zip_bytes: bytes) -> dict:
        ...
        for row in reader:
            try:
                ...
                site_addr  = r.get("site_addr_1", "")
                site_city  = r.get("site_addr_2", "") or "HOUSTON"
                site_state = "TX"
                site_zip   = r.get("site_addr_3", "")
                
                # If site_addr_1 is empty, build from str components
                if not site_addr:
                    parts = [
    ...
    def _load_parcel_tieback(self, zip_bytes: bytes) -> dict:
        ...
        log.info(f"  parcel_tieback fields: {fields[:15]}")
        for row in reader:
            try:
                r    = {k.strip().lower(): (v or "").strip()
                        for k, v in row.items() if k}
                ...
                if not acct:
                    continue
                result[acct] = {
                    "subdivision": r.get("subdv_cd","") or r.get("subdv","") or r.get("subdivision",""),
                    "section":     r.get("sec","") or r.get("section",""),
                    "lot":         r.get("lot","") or r.get("lot_nbr",""),
                    "block":       r.get("block","") or r.get("block_nbr",""),
                }
            except Exception:
                continue
        log.info(f"  Parcel tieback: {len(result)} records")
        return result
        ...
        for tr in all_trs:
            try:
                ...
                if name_cell:
                    ...
                    # Fallback: parse flat text if nested tables didn't work
                    if not grantor:
                        for line in full_text.replace('Grantor', '\nGrantor').replace('Grantee', '\nGrantee').split('\n'):
                            line = line.strip()
                            if not line:
                                continue
                            low = line.lower()
                            if low.startswith('grantor'):
                                grantor = re.sub(r'^grantor\s*:\s*', '', line, flags=re.IGNORECASE).strip()
                            elif low.startswith('grantee'):
                                g = re.sub(r'^grantee\s*:\s*', '', line, flags=re.IGNORECASE).strip()
                                if g:
                                    grantees.append(g)

                # Cell +4: Legal description
                legal_cell = _gc(4)
                legal = legal_cell.get_text(' ', strip=True) if legal_cell else ''
