...
class EnrichmentEngine:
    ...
    def enrich(self, rec):
        ...
        # Strategy 2: Name match — with length-based confidence
        # Longer normalized names = more specific = higher confidence
        parsed_legal = parse_legal_description(legal_raw)
        
        for target in get_match_targets(rec):
            norm_target = normalize_name(target)
            norm_words  = len(norm_target.split()) if norm_target else 0
            ...
            for v in name_variants(target):
                ...
                # Confidence based on name specificity + match count
                if len(matches) == 1:
                    if v_words >= 3:
                        conf = "MEDIUM"
                    else:
                        # Short match — cross-validate with legal description
                        # Only accept if we can narrow to 1 parcel via legal
                        conf = "LOW"
                    reason = f"Name match ({v_words}w) — unique: {v}"
                    return self._apply(rec, matches[0], conf, reason)
                elif len(matches) <= 5 and v_words >= 3:
                    # Few matches, specific name — take best one
                    # Prefer match where address city matches legal subdivision
                    sub = parsed_legal.get("subdivision", "")
                    best = matches[0]
                    for m in matches:
                        site = m.get("site_addr", "").upper()
                        if sub and sub[:6] in site:
                            best = m
                            break
                    reason = f"Name match — {len(matches)} parcels, best selected: {v}"
                    return self._apply(rec, best, "LOW", reason)
                elif len(matches) <= 3 and v_words >= 4:
                    reason = f"Name match — {len(matches)} parcels: {v}"
                    return self._apply(rec, matches[0], "LOW", reason)

        # Strategy 3: Subdivision name from legal description
        parsed    = parsed_legal if 'parsed_legal' in dir() else parse_legal_description(legal_raw)
        sub       = parsed.get("subdivision", "").strip()
