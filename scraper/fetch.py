...
class HarrisClerkScraper:
        ...
        for year, month in months_to_search:
            for search_type in ["Sale Date", "File Date"]:
                try:
                    ...
                    html = await page.content()
                    rows = self._parse_frcl_page(html, year, month, search_type)
                    frcl_count += len(rows)
                    self.records.extend(rows)
                    log.info(f"    FRCL [{search_type}] {month_names[month]} {year}: {len(rows)} foreclosures")
    ...
    def _parse_frcl_page(self, html: str, year: int, month: int, search_type: str = "Sale Date") -> list:
        ...
        for ri, tr in enumerate(best_table.find_all("tr")):
            ...
            # Positional fallback (common FRCL layout):
            # col 0: doc_id, col 1: grantor, col 2: sale_date, col 3+: address/details
            doc_num  = get_field("DOCUMENT", "DOC", "INSTRUMENT") or (cells[0] if cells else "")
            grantor  = get_field("GRANTOR", "NAME", "OWNER") or (cells[1] if len(cells) > 1 else "")
            sale_dt  = get_field("SALE DATE", "SALE", "DATE") or (cells[2] if len(cells) > 2 else f"{year}-{month:02d}-01")
            address  = get_field("ADDRESS", "PROPERTY", "LEGAL") or (cells[3] if len(cells) > 3 else "")
