...
CLERK_FRCL_URL = "https://www.cclerk.hctx.net/applications/websearch/FRCL_R.aspx"  # Foreclosures

DOC_TYPES = {
    "L/P":    ("lp",      "Lis Pendens"),
    "REL":    ("lp",      "Release of Lien / Lis Pendens"),
    "NOTICE": ("fc",      "Notice (Foreclosure/Tax/Trustee Sale)"),
    ...
    "LEVY":   ("lien",    "Notice of Levy on Real Estate"),
}
        ...
        for attempt in range(3):
            try:
                ...
                if attempt == 2:
                    return
        try:
            # Harris County RP.aspx real form field names
            # Instrument Type dropdown
            instr_selectors = [
                'select[name*="InstrumentType"]',
                'select[id*="InstrumentType"]',
                'select[name*="ddlInstrType"]',
                'select[id*="ddlInstrType"]',
            ...
            for sel in instr_selectors:
                try:
                    ...
                    if el:
                        ...
                        log.info(f"    Set instrument type via: {sel}")
                        break
                except Exception:
                    pass
            ...
            # Date fields — Harris County RP form
            date_selectors = [
                ('input[name*="DateFrom"]',  date_from),
                ('input[id*="DateFrom"]',    date_from),
                ('input[name*="BeginDate"]', date_from),
                ('input[name*="StartDate"]', date_from),
                ('input[name*="DateTo"]',    date_to),
                ('input[id*="DateTo"]',      date_to),
                ('input[name*="EndDate"]',   date_to),
                ('input[name*="StopDate"]',  date_to),
            ]
            for sel, val in date_selectors:
                try:
                    el = await page.query_selector(sel)
                    if el:
...
class HarrisClerkRequestsScraper:
    SESSION_URL = "https://www.cclerk.hctx.net/applications/websearch/RP.aspx"
