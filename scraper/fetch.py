...
class HCADParcelLookup:
    KNOWN_URLS = [
        ...
        "gdrive:1wV4EW-uxasZUjkc_wxOr-UjVKUTK5-qh",
        # HCAD fallbacks
...
class HarrisClerkScraper:
    ...
    async def _search_one(self, page, doc_code: str,
        ...
        try:
            ...
            # EXACT field names from debug log:
            ...
            try:
                ...
                if el:
                    ...
                    log.info(f"    Filled instrument: {doc_code}")
            except Exception as e:
                log.warning(f"    Instrument field error: {e}")
            ...
            try:
                ...
                log.warning(f"    Date from error: {e}")
            ...
            while True:
                ...
                # Debug: dump first result page HTML snippet on first doc type
                if page_num == 1 and doc_code == "L/P":
                    from bs4 import BeautifulSoup as _BS
                    _soup = _BS(html, "lxml")
                    # Find all tables and log their headers
                    _tables = _soup.find_all("table")
                    log.info(f"    DEBUG: {len(_tables)} tables found on page")
                    for _i, _t in enumerate(_tables[:5]):
                        _txt = _t.get_text(" ", strip=True)[:200]
                        log.info(f"    Table {_i}: {_txt}")
